...
export async function getAdvancedOffers(firestoreCollectionName: string) {
  const offerRef = db.collection(firestoreCollectionName)
...
export async function removeAdvancedOffers(offers: any[], firestoreCollectionName: string) {
  for (const offer of offers) {
    const offerRef = db.collection(firestoreCollectionName).doc(offer.order_hash)
    await offerRef.delete()
  }
}

export async function setAdvancedOffers(offers: any[], firestoreCollectionName: string) {
  for (const offer of offers) {
    const offerRef = db.collection(firestoreCollectionName).doc(offer.order_hash)
